...
class LogGatewayImpl(
    ...
    override suspend fun getNetworkLog(): Flow<List<LogNetworkMessage>> = withContext(dispatcherProvider.io) {
        logNetworkMessageDao.getAll().map { list ->
            list.map { it.toLogNetworkMessage() }
        }
    }
